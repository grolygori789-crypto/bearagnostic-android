(() => {
  ...
  function renderResults() {
    ...
    state.bulkEligible = allFiltered;
  ...
  function syncBulkSelectionSummary() {
    const host = byId('baInstallersSelection'); if (!host) return;
    ...
    const strong = host.querySelector('.ba-installers-selection__copy strong');
    ...
    const clear = host.querySelector('[data-installer-action="clear"]'); if (clear) clear.disabled = count===0;
    const review = host.querySelector('[data-installer-action="review"]'); if (review) review.disabled = !(count && isFresh() && !state.scannerRunning);
  }
  ...
  function finishBulkSelection() {
    const surface = byId('baInstallersSurface'); if (surface) surface.dataset.finalBulkPainting='1';
    state.message = '';
