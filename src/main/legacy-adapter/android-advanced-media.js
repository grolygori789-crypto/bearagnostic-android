(() => {
  ...
  // Bulk operations update state in one pass and preserve loaded media/previews.
  function syncBulkSelectionSummary(){const host=byId('baMediaSelection');if(!host)return;const chosen=selectedItems();const count=chosen.length;const bytes=chosen.reduce((sum,i)=>sum+n(i.sizeBytes),0);const strong=host.querySelector('.ba-media-selection__inner>div:first-child strong');if(strong){const value=c().selected.replace('{count}',count).replace('{size}',formatBytes(bytes));if(strong.firstChild&&strong.childNodes.length===1)strong.firstChild.nodeValue=value;else strong.textContent=value;}const clear=host.querySelector('[data-media-action="clear"]');if(clear)clear.disabled=count===0;const review=host.querySelector('[data-media-action="review"]');if(review)review.disabled=!(count&&fresh());}

  function bulkCandidates(){return Array.isArray(state.bulkEligible)?state.bulkEligible:filteredItems();}
  function bulkStatus(){const eligible=bulkCandidates();const selectedEligible=eligible.reduce((count,item)=>count+(state.selected.has(String(item.id))?1:0),0);return{eligibleCount:eligible.length,selectedEligibleCount:selectedEligible,selectedTotal:state.selected.size,allSelected:eligible.length>0&&selectedEligible===eligible.length,capReached:state.selected.size>=MAX_DELETE_SELECTION};}
  function syncBulkSelectionDom(){const surface=byId('baMediaSurface');if(!surface||surface.hidden)return;surface.querySelectorAll('[data-media-id]').forEach((box)=>{const id=String(box.dataset.mediaId||'');const checked=Boolean(id&&state.selected.has(id));if(box.checked!==checked)box.checked=checked;box.closest('.ba-media-row')?.classList.toggle('is-selected',checked);});}
  function finishBulkSelection(){const surface=byId('baMediaSurface');if(surface)surface.dataset.finalBulkPainting='1';state.message='';syncBulkSelectionDom();syncBulkSelectionSummary();requestAnimationFrame(()=>{if(surface)delete surface.dataset.finalBulkPainting;requestAnimationFrame(()=>window.BearagnosticFinalPolish?.syncBulkControls?.());});return bulkStatus();}
  function bulkSelectAll(){if(!state.open||state.view!=='results'||!state.summary?.available||!fresh())return bulkStatus();for(const item of bulkCandidates()){const id=String(item?.id||'');if(!id||state.selected.has(id))continue;if(state.selected.size>=MAX_DELETE_SELECTION){state.message=c().limitReached;break;}state.selected.add(id);}return finishBulkSelection();}
  function bulkClear(){if(state.selected.size)state.selected.clear();return finishBulkSelection();}
  ...
  window.BearagnosticAdvancedMedia=Object.freeze({build:BUILD,open,close,refresh,bulkSelectAll,bulkClear,bulkStatus});
})();
