(() => {
  ...
  function selectAllThroughExistingHandlers(key) {
    ...
    try {
      ...
      // The owning tool performs a post-paint control sync. Do not run the
      // global polish pass synchronously in the tap path.
    }
  }
  ...
  function initialize() {
    ...
    let bulkObserverFrame = 0;
    const observer = new MutationObserver(() => {
      if (document.querySelector('[data-final-bulk-painting="1"]')) {
        if (bulkObserverFrame) return;
        bulkObserverFrame = requestAnimationFrame(() => {
          bulkObserverFrame = requestAnimationFrame(() => {
            bulkObserverFrame = 0;
            schedulePatch();
          });
