(() => {
  ...
  function syncBulkSelectionSummary() {
    const host = byId('baDownloadsSelection'); if (!host) return;
    ...
    const strong = host.querySelector('.ba-downloads-selection__copy strong');
    ...
    const clear = host.querySelector('[data-download-action="clear"]'); if (clear) clear.disabled = count===0;
    const review = host.querySelector('[data-download-action="review"]'); if (review) review.disabled = !(count && isFresh() && !state.scannerRunning);
  }
  ...
  function finishBulkSelection() {
    const surface = byId('baDownloadsSurface'); if (surface) surface.dataset.finalBulkPainting='1';
    state.message = '';
