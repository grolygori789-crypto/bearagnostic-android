(() => {
  ...
  function syncBulkSelectionSummary() {
    const host = byId('baZeroSelection'); if (!host) return;
    ...
    const strong = host.querySelector('.ba-zero-selection__copy strong');
    ...
    const clear = host.querySelector('[data-zero-action="clear"]'); if (clear) clear.disabled = count===0;
    const review = host.querySelector('[data-zero-action="review"]'); if (review) review.disabled = !(count && isFresh() && !state.scannerRunning);
  }
  ...
  function syncBulkSelectionDom() {
    const surface = document.querySelector('#baZeroSurface');
    if (!surface || surface.hidden) return;
    surface.querySelectorAll('[data-zero-id]').forEach((box) => {
      const id = String(box.dataset.zeroId || '');
      const checked = Boolean(id && state.selected.has(id));
      if (box.checked !== checked) box.checked = checked;
      box.closest('.ba-zero-file')?.classList.toggle('is-selected', checked);
    });
  }
  function finishBulkSelection() {
    const surface = byId('baZeroSurface'); if (surface) surface.dataset.finalBulkPainting='1';
    state.message = '';
