...
def main():
    ...
    require('versionCode = 50' in build, 'B50 versionCode missing')
    require('versionName = "0.35.2-alpha50"' in build, 'B50 versionName missing')
    cache_versions = set(re.findall(r'android-[a-z0-9-]+\.js\?v=(\d+)', build))
    require(cache_versions == {'50'}, f'Android adapter cache incoherent: {sorted(cache_versions)}')
    ...
    require('const BUILD = 50;' in locale, 'B50 locale-polish metadata missing')
    ...
    require('BRIDGE_VERSION = 16' in bridge, 'B50 unexpectedly changed the native bridge contract')
    ...
    for token in (
        ...
        require(token in ui, f'Missing B50 Dev Mode UX contract: {token}')
    ...
    for token in (
        ...
        require(token in ui, f'B50 Developer Mode entry hardening missing: {token}')
    ...
    # Real Android WebView JavaScriptInterface methods must be invoked as members of the
    # bridge object. B49 detached NATIVE[name] into a local function and physical-device
    # testing proved that the receiver loss throws a bridge error.
    require('const fn=NATIVE[name]' not in ui and 'const fn = NATIVE[name]' not in ui,
            'Detached WebView bridge method invocation reintroduced')
    require("NATIVE[name](Boolean(value))" in ui,
            'Boolean owner-console calls do not preserve the WebView bridge receiver')
    require("NATIVE[name](String(value))" in ui,
            'String owner-console calls do not preserve the WebView bridge receiver')
    require("NATIVE[name]()" in ui,
            'No-argument owner-console calls do not preserve the WebView bridge receiver')
    require("detail:String(error?.name||'Error').slice(0,40)" in ui,
            'Bridge diagnostics do not preserve a bounded JS error class')

    require('Verify B50 WebView bridge binding' in workflow,
            'CI does not run the B50 verifier')

    # B50 is an isolated WebView bridge receiver fix; scanner and Insights evidence collection are forbidden.
    ...
    require(not (package_paths & forbidden), f'B50 package touches evidence/scanner files: {package_paths & forbidden}')

    if args.release_apk:
        verify_release_apk(args.release_apk)

    print('B50 WebView bridge binding contracts: PASS')
...
if __name__ == '__main__':
    try:
        ...
        print(f'B50 dual-sided billing sandbox contracts: FAIL — {exc}', file=sys.stderr)
        sys.exit(1)
