...
def main():
    ...
    require('versionCode = 48' in build, 'B48 versionCode missing')
    require('versionName = "0.35.0-alpha48"' in build, 'B48 versionName missing')
    cache_versions = set(re.findall(r'android-[a-z0-9-]+\.js\?v=(\d+)', build))
    require(cache_versions == {'48'}, f'Android adapter cache incoherent: {sorted(cache_versions)}')
    ...
    require('const BUILD = 48;' in locale, 'B48 locale-polish metadata missing')
    ...
    require('BRIDGE_VERSION = 16' in bridge, 'Bridge version not bumped for dual-sided B48')
    ...
    for token in (
        ...
        require(token in ui, f'Missing B48 Dev Mode UX contract: {token}')
    ...
    require('Verify B48 dual-sided billing sandbox' in workflow,
            'CI does not run the B48 verifier')

    # B48 is monetization QA only; scanner and Insights evidence collection are forbidden.
    ...
    require(not (package_paths & forbidden), f'B48 package touches evidence/scanner files: {package_paths & forbidden}')

    if args.release_apk:
        verify_release_apk(args.release_apk)

    print('B48 dual-sided billing sandbox contracts: PASS')
...
if __name__ == '__main__':
    try:
        ...
        print(f'B48 dual-sided billing sandbox contracts: FAIL — {exc}', file=sys.stderr)
        sys.exit(1)
