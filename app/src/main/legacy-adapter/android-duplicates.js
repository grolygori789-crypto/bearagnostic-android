(() => {
  ...
  const COPY = {
    en: {
      ...
      noDupTitle:'No exact duplicate groups verified', noDupBody:'No byte-for-byte duplicate group was verified in this scan’s duplicate coverage. Similar files are intentionally not treated as duplicates.', checkAgain:'Check again', done:'Done', close:'Close', selectionPrompt:'Choose extra copies to remove', selectionPicked:'{count} copies selected',
    ...
    ja: {
      ...
      noDupTitle:'検証できた完全一致の重複グループはありません', noDupBody:'この検証範囲ではバイト単位で同一の重複グループは確認されませんでした。似ているだけのファイルは重複扱いしません。', checkAgain:'再チェック', done:'完了', close:'閉じる', selectionPrompt:'削除する余分なコピーを選択', selectionPicked:'{count} 件選択',
  ...
  function hiddenBadge(){ return lang()==='th'?'ซ่อน':lang()==='ja'?'非表示':'HIDDEN'; }
