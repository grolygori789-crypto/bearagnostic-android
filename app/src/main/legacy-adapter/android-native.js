(() => {
  ...
  function ensureNativePreferences(){
    ...
    const version=String(nativeState?.versionName||'0.17.0-alpha17').replace('-debug','');
    ext.innerHTML=`
      ...
      <section class="native-pref-card">
        ...
        <div class="native-pref-rows"><div class="native-pref-row"><span>${c.version}</span><b class="slate">${version} · B17</b></div></div>
      </section>`;
  }
  ...
  window.addEventListener('bearagnostic:screenchange',(event)=>{const screen=event.detail?.screen||'home';syncHomeButton(screen);if(screen==='preferences')ensureNativePreferences();});

  document.addEventListener('DOMContentLoaded',()=>{
    renderModeSheet(); ensureActionSurfaces();
    parseNativeState(); syncHomeButton('home'); ensureNativePreferences();
    const buildLabel=document.querySelector('.app-footer__build'); if(buildLabel) buildLabel.textContent='v0.17 · B17';
  ...
  window.addEventListener('bearagnostic:languagechange',()=>{if(!byId('nativeModeSheet')?.hidden)renderModeSheet();setScanState(running?'running':lastProgress?.state==='complete'?'complete':'idle');syncHomeButton();ensureNativePreferences();if(lastCompleteResult&&!byId('nativeResultsSheet')?.hidden)openResults(lastCompleteResult);});
})();
